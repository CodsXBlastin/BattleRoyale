...
$this->getServer()->getLevelByName($this->data['arena']['arena_world'])->setTime(str_replace(['day', 'night'], [6000, 18000], $this->data['arena']['time'])); 
$this->getServer()->getLevelByName($this->data['arena']['arena_world'])->stopTime(); } } public function enableScheduler(){ 
...
if($b->x == $this->data["signs"]["join_sign_x"] && $b->y == $this->data["signs"]["join_sign_y"] && $b->z == $this->data["signs"]["join_sign_z"] && $b->level == $this->getServer()->getLevelByName($this->data["signs"]["join_sign_world"])){ 
if($this->getPlayerMode($p) === 0 || $this->getPlayerMode($p) === 1 || $this->getPlayerMode($p) === 2){ 
return; 
} 
$this->joinToArena($p); 
}
if($b->x == $this->data["signs"]["return_sign_x"] && $b->y == $this->data["signs"]["return_sign_y"] && $b->z == $this->data["signs"]["return_sign_z"] && $b->level == $this->getServer()->getLevelByName($this->data["arena"]["arena_world"])){
...
if($this->setup === true){ 
$p->sendMessage($this->getPrefix().$this->getMsg('arena_in_setup')); 
return; 
} 
if(count($this->lobbyp) >= $this->getMaxPlayers()){ 
$p->sendMessage($this->getPrefix().$this->getMsg('game_full')); 
return;
}
if($this->game === 1){
return; 
}
if(!$this->getServer()->isLevelGenerated($this->data['arena']['arena_world'])){
$this->getServer()->generateLevel($this->data['arena']['arena_world']); 
}
if(!$this->getServer()->isLevelLoaded($this->data['arena']['arena_world'])){ 
$this->getServer()->loadLevel($this->data['arena']['arena_world']); 
} 
$this->saveInv($p); $p->teleport(new Position($this->data['arena']['lobby_position_x'], $this->data['arena']['lobby_position_y'], $this->data['arena']['lobby_position_z'], $this->getServer()->getLevelByName($this->data['arena']['lobby_position_world']))); 
$p->sendMessage($this->getPrefix().$this->getMsg('join'));
$this->lobbyp[strtolower($p->getName())] = $p; 
$p->addTitle(TextFormat::GREEN . "BattleRoyale", "", 20, 40, 20); 
$p->removeAllEffects(); 
$vars = ['%1'];
$replace = [$p->getName()]; 
$this->messageArenaPlayers(str_replace($vars, $replace, $this->getMsg('join_others')));
return;
$p->sendMessage($this->getPrefix().$this->getMsg('has_not_permission'));  
} 
public function leaveArena(Player $p) { 
if($this->getPlayerMode($p) == 0){ 
unset($this->lobbyp[strtolower($p->getName())]); 
$p->teleport(new Position($this->data['arena']['leave_position_x'], $this->data['arena']['leave_position_y'], $this->data['arena']['leave_position_z'], $this->getServer()->getLevelByName($this->data['arena']['leave_position_world']))); 
$p->setSpawn(new Position($this->data['arena']['leave_position_x'], $this->data['arena']['leave_position_y'], $this->data['arena']['leave_position_z'], $this->getServer()->getLevelByName($this->data['arena']['leave_position_world']))); 
} 
if($this->getPlayerMode($p) == 1){ 
unset($this->ingamep[strtolower($p->getName())]);
$p->teleport(new Position($this->data['arena']['leave_position_x'], $this->data['arena']['leave_position_y'], $this->data['arena']['leave_position_z'], $this->getServer()->getLevelByName($this->data['arena']['leave_position_world']))); 
$p->setSpawn(new Position($this->data['arena']['leave_position_x'], $this->data['arena']['leave_position_y'], $this->data['arena']['leave_position_z'], $this->getServer()->getLevelByName($this->data['arena']['leave_position_world']))); 
$this->messageArenaPlayers(str_replace("%1", $p->getName(), $this->getMsg('leave_others'))); $this->checkAlive(); 
}
...
if(!$this->getServer()->isLevelGenerated($this->data['arena']['leave_position_world'])){ $this->getServer()->generateLevel($this->data['arena']['leave_position_world']); 
} 
if(!$this->getServer()->isLevelLoaded($this->data['arena']['leave_position_world'])){ 
$this->getServer()->loadLevel($this->data['arena']['leave_position_world']); 
} 
$p->sendMessage($this->getPrefix().$this->getMsg('leave')); 
...
$p->setGamemode(0); 
$level = $this->getServer()->getLevelByName($this->data['arena']['arena_world']); $level->setBlockIdAt($this->data['arena']['join_position_x'], $this->data['arena']['join_position_y'] - 1, $this->data['arena']['join_position_z'], 57); 
...
$level->setBlockIdAt($this->data['arena']['join_position_x'], $this->data['arena']['join_position_y'] - 1, $this->data['arena']['join_position_z'] - 1, 57); 
$p->teleport(new Position($this->data['arena']['join_position_x'] + 0.5, $this->data['arena']['join_position_y'], $this->data['arena']['join_position_z'] + 0.5, $this->getServer()->getLevelByName($this->data['arena']['arena_world']))); 
$p->setSpawn(new Position($this->data['arena']['join_position_x'] + 0.5, $this->data['arena']['join_position_y'], $this->data['arena']['join_position_z'] + 0.5, $this->getServer()->getLevelByName($this->data['arena']['arena_world']))); 
$p->setImmobile(true);
} 
$this->messageArenaPlayers($this->getMsg('start_game')); 
} 
...
if($this->getServer()->isLevelLoaded($this->data['arena']['arena_world'])){
$this->getServer()->unloadLevel($this->getServer()->getLevelByName($this->data['arena']['arena_world'])); 	
} 
$zip = new \ZipArchive; 
$zip->open($this->getDataFolder() . 'arenasmap/' . $this->data['arena']['arena_world'] . '.zip'); 
...
$p->getInventory()->removeItem($itemName); 
$level = $this->getServer()->getLevelByName($this->data['arena']['arena_world']); $yaw = $p->yaw;
if($yaw >= 225 && $yaw <= 315){
$x = $p->x + 3; 
...
$pl->sendMessage($this->getPrefix().str_replace(['%2', '%1'], [count($this->ingamep), $p->getName()], $this->getMsg('death'))); 
} 
$this->checkAlive(); 
} 
} 
...
$players[strtolower($p)]->sendMessage(str_replace("%1", $reason, $this->getMsg('kick_from_game')));
...
if(!$this->getServer()->isLevelGenerated($this->data['signs']['join_sign_world'])){ 
$this->getServer()->generateLevel($this->data['signs']['join_sign_world']); 
} 
if(!$this->getServer()->isLevelLoaded($this->data['signs']['join_sign_world'])){ 
$this->getServer()->loadLevel($this->data['signs']['join_sign_world']); 
} 
if(!$this->getServer()->isLevelGenerated($this->data['arena']['leave_position_world'])){ 
$this->getServer()->generateLevel($this->data['arena']['leave_position_world']); 
} 
if(!$this->getServer()->isLevelLoaded($this->data['arena']['leave_position_world'])){ 
$this->getServer()->loadLevel($this->data['arena']['leave_position_world']); 
} 
